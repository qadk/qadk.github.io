...
tags: ["PHP", "Architecture", "Ubiquitous Language", "Refactoring"]
description: "用 Interface 與多型拆解倉庫上架系統的 if-else，以 DDD Ubiquitous Language 的精神建模，讓新增商品類型只需要最小幅度的改動"
cover:
...
從商品出發：先問 `requirements()` 知道能去哪些櫃型，再從那些櫃型的 `allowedZones()` 推出可接受的區域，最後由區域的 `acceptedProductTypes()` 確認自己接不接受這個商品。整個過程都是純規則推導，不碰資料庫——等推導完了再去查實體儲位，不必要的 I/O 全部省掉

## 這樣改之後
拆開之後還有一個好處——程式碼說的話跟物流人員說的話是同一套

舉個例子：今天要新增「雨傘」類型，只需要問物流人員這類型需要放在什麼櫃型又要放在什麼區域裡，即使是新人工程師也能輕鬆上手完成這個需求：
...
這個設計的起點，其實是 DDD 裡 **Ubiquitous Language（通用語言）** 的概念——用物流人員理解的詞彙去建模，讓程式碼說的話跟現場說的話是同一套
